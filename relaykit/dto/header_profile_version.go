...
func resolveAICodingCLIBaseProfileID(profile HeaderProfile, meta *HeaderProfileVersionMeta) string {
	...
	// Prefer the profile's own id: several profiles share one npm package
	// (for example codex-cli and codex-compatible), and each has its own UA
	// shape, so a package-name lookup alone is ambiguous.
	profileID := strings.TrimSpace(profile.ID)
	if baseProfileID, _, ok := strings.Cut(profileID, "@"); ok {
		profileID = strings.TrimSpace(baseProfileID)
	}
	if _, known := builtinAICodingCLIPackageNames[profileID]; known {
		return profileID
	}

	if meta != nil {
		// Fall back to a package-name lookup for snapshots that only carry a
		// package. Iterate a fixed order so the result is deterministic.
		packageName := strings.TrimSpace(meta.PackageName)
		for _, candidate := range aiCodingCLIPackageLookupOrder {
			if builtinAICodingCLIPackageNames[candidate] == packageName {
				return candidate
			}
		}
	}
...
// aiCodingCLIPackageLookupOrder keeps the package-name fallback deterministic
// and prefers the CLI profiles, which is the historical behavior for snapshots
// that predate the protocol-compatible templates.
var aiCodingCLIPackageLookupOrder = []string{
	"codex-cli",
	"claude-code",
	"gemini-cli",
	HeaderProfileIDCodexCompatible,
	HeaderProfileIDClaudeCodeCompatible,
}
...
func buildAICodingCLIUserAgent(profileID string, version string, platform string) string {
	...
	// counterparts; only the UA shape differs, so they track latest too.
	case HeaderProfileIDCodexCompatible:
		return fmt.Sprintf("codex_cli_rs/%s", version)
	case HeaderProfileIDClaudeCodeCompatible:
