...
type ChannelMutateDrawerProps = {
  ...
  instanceId?: string
  mode?: 'create' | 'edit'
  presentation?: 'sheet' | 'window'
  onSuccessClose?: () => void
  onDirtyChange?: (dirty: boolean) => void
}

export type ChannelEditorWindowContentProps = {
  instanceId: string
  mode: 'create' | 'edit'
  channelId: number | null
  onRequestClose: () => void
...
function ChannelEditorSurface(props: {
  children: ReactNode
  presentation: 'sheet' | 'window'
}) {
  if (props.presentation === 'window') {
    return (
      <div className='bg-background text-foreground flex h-full w-full flex-col gap-0 overflow-hidden'>
        {props.children}
      </div>
    )
  }
...
function ChannelEditorTitle(props: {
  children: ReactNode
  presentation: 'sheet' | 'window'
}) {
  if (props.presentation === 'window') {
    return <h3 className='flex items-center gap-3'>{props.children}</h3>
  }
...
function ChannelEditorDescription(props: {
  children: ReactNode
  presentation: 'sheet' | 'window'
}) {
  if (props.presentation === 'window') {
    return (
      <p className='text-muted-foreground mt-1 text-sm'>{props.children}</p>
    )
  }

  return <SheetDescription className='mt-1'>{props.children}</SheetDescription>
}
...
export function ChannelMutateDrawer(props: ChannelMutateDrawerProps) {
  ...
  const channels = useOptionalChannels()
  const setOpen = channels?.setOpen
  ...
  const isEditing = mode ? mode === 'edit' : Boolean(currentRow)
  const channelId = requestedChannelId ?? currentRow?.id ?? null
  ...
  const formId = instanceId ? `channel-form-${instanceId}` : 'channel-form'
  const getEditorElementId = useCallback(
    (id: string) => (instanceId ? `${id}-${instanceId}` : id),
    [instanceId]
  )
  ...
  const channelMutation = useChannelMutateForm({
    currentRow: effectiveCurrentRow,
    channelId,
    isEditing,
  ...
  const handleEditorNavNavigate = useCallback(
    (targetId: string) => {
      ...
      const scrollTargetIntoView = () => {
        document
          .querySelector<HTMLElement>(`#${getEditorElementId(targetId)}`)
    ...
    [advancedSettingsOpen, getEditorElementId, handleAdvancedSettingsOpenChange]
  )
  ...
  const handleOpenChange = useCallback(
    (v: boolean) => {
      onOpenChange(v)
      if (!v && presentation === 'window') return
    ...
    [form, onOpenChange, presentation]
  )
  ...
  const editorFooter =
    presentation === 'sheet' ? (
      ...
      <div className={cn(sideDrawerFooterClassName(), 'shrink-0')}>
        <Button
          type='button'
          variant='outline'
  ...
  const editorContent = (
    <ChannelEditorSurface presentation={presentation}>
      {presentation === 'sheet' ? (
        <SheetHeader className={sideDrawerHeaderClassName()}>
          {editorHeader}
        </SheetHeader>
      ) : (
        <div className='border-border/70 bg-background/95 supports-[backdrop-filter]:bg-background/80 shrink-0 border-b px-4 py-3 backdrop-blur sm:px-6 sm:py-4'>
          {editorHeader}
        </div>
      )}
        ...
        >
          {isChannelDetailLoading ? (
            ...
            <div className='grid gap-5 lg:grid-cols-[13rem_minmax(0,1fr)] lg:items-start'>
              <ChannelEditorNav
                providerLogo={<ChannelTypeLogo type={currentType} size={18} />}
              ...
              <div className='flex min-w-0 flex-col gap-5'>
                {/* ── Basic Information ── */}
                <div
                  id={getEditorElementId(CHANNEL_EDITOR_SECTION_IDS.identity)}
                ...
                {/* ── Models & Groups ── */}
                <div
                  id={getEditorElementId(CHANNEL_EDITOR_SECTION_IDS.models)}
                  className='scroll-mt-4'
                >
                  <ChannelModelsSection>
                ...
                <div
                  id={getEditorElementId(CHANNEL_EDITOR_SECTION_IDS.advanced)}
                  className='scroll-mt-4'
                >
                  <ChannelAdvancedSection
...
export function ChannelEditorWindowContent(
  props: ChannelEditorWindowContentProps
) {
  const channels = useOptionalChannels()
  ...
  if (!channels) {
    return (
      <ChannelsProvider>
        <ChannelEditorWindowContentBody {...props} />
      </ChannelsProvider>
    )
  }

  return <ChannelEditorWindowContentBody {...props} />
}

function ChannelEditorWindowContentBody(
  props: ChannelEditorWindowContentProps
) {
  return (
    <ChannelMutateDrawer
      ...
      onOpenChange={(nextOpen) => {
        if (!nextOpen) props.onRequestClose()
      }}
